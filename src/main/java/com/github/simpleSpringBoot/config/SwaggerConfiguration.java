package com.github.simpleSpringBoot.config;
...
public class SwaggerConfiguration {
	...
	public Docket api() {
		...
		return new Docket(DocumentationType.SWAGGER_2).host(host).pathProvider(new CustomPathProvider()).select()
				.apis(RequestHandlerSelectors.any()).paths(PathSelectors.regex("/api/v1/.*")).build();
	}
